...
begin
  //Atualiza o query segundo o que está escrito e marcado no checkbox
  //Se Todos
  ...
  begin
    //Seleciona requisitos com estado em avaliação
    with query do
    begin
      query.close;
      query.sql.clear;
      query.sql.text:=('select nome as Nome from tbrequisito where nome like "'+edtProcurar.text+'%" and estado="EM AVALIAÇÃO" and codigo_projeto=(select codigo_projeto from tbprojeto acesso order by codigo desc limit 1)');
      query.open;
    end;
  end;
  //Se em implementação
  ...
  begin
    //Seleciona requisitos com estado em implementação
    with query do
    begin
      query.close;
      query.SQL.Clear;
      query.sql.text:=('select nome as Nome from tbrequisito where nome like "'+edtProcurar.text+'%" and estado="EM IMPLEMENTAÇÃO" and codigo_projeto=(select codigo_projeto from tbprojeto acesso order by codigo desc limit 1)');
      query.open;
    end;
  end;
  //Se implementado
...
begin
  ...
  edtProcurar.Clear;
  //Atualiza lista de requisito com estado Em Avaliação
  with query do
  begin
    query.close;
    query.sql.clear;
    query.sql.text:=('select nome as Nome from tbrequisito where estado="EM AVALIAÇÃO" and codigo_projeto=(select codigo_projeto from tbprojetoacesso order by codigo desc limit 1)');
    query.open;
  end;
  //Seta o cursor no edtProcurar
  edtProcurar.SetFocus;
...
begin
  ...
  //Limpa edtProcurar
  edtProcurar.Clear;
  //Atualiza lista de requisitos com estado em implementação
  with query do
  begin
    query.close;
    query.sql.clear;
    query.sql.text:=('select nome as Nome from tbrequisito where estado="EM IMPLEMENTAÇÃO" and codigo_projeto=(select codigo_projeto from tbprojetoacesso order by codigo desc limit 1)');
    query.Open;
  end;
...
var
  //Variável que irá pegar o nome do requisito do gridRequisitos
...
begin
  ...
  begin
    ...
    //Executa impressao de em Avaliação
  if (rbtnAvaliacao.Checked=True) then
  begin
...
begin
  //Libera botão Imprimir Unidade ou não
  if (edtCod.Text='') then
  begin
